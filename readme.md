# Cборка Webpack для многостраничной верстки
В сборке используется синтаксис `JSX`, но это не `React`! `JSX` используется в качестве шаблонизатора. В компонентах не работают React-хуки, `console.log()` и прочий интерактив, т.к. весь `JSX` в конечном итоге превращается в статичную html-разметку. 

Однако все же есть возможность отрендерить интерактивный `React` компонент на статичной `html` странице. См. `src/widgets/reactComponentExample`

Все скрипты должны импортироваться и запускаться в точке входа `src/app/app.js`.
